...
### Basic Usage
```bash
# Analyze a GIF file (same as default behavior)
...
# Print optimization report (duplicate frames / transparency fixes / palette normalization)
moon run src/main optimize gif/__Attack.gif

# Encode a demo GIF and save to a path (default: encoded_demo.gif)
moon run src/main encode-demo out/demo.gif

# Parse summary with options (strict parsing on/off)
moon run src/main summary gif/__Attack.gif false

# Directly passing a path still runs analyze
...
- **Command-line demo** – `moon run src/main encode-demo [path]` 生成一个 32×32 的渐变 GIF；`moon run src/main optimize your.gif` 会打印优化统计。
...
- **CLI 体验** – `moon run src/main stream your.gif 4096` 能实时输出 Header / 图像帧 / 扩展信息，`moon run src/main summary your.gif true` 可以使用 `GifParseOptions` 输出结构摘要。
